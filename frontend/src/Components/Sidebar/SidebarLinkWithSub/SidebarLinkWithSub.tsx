...
const SidebarLinkWithSub: React.FC<sidebarLinkWithSubType> = ({icon, label}) => {
    const [isOpen, setOpen] = React.useState<boolean>(false);
        return (
            <div className={`sidebarLinkWithSub ${isOpen ? 'activeLink' : ""}`} onClick={(e: React.MouseEvent) => {
                setOpen(!isOpen)
            }}>
                            <div className="icon">
                            ...
                            <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" className={`arrow ${isOpen ? 'activeLink' : ""}`} height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><polyline points="9 18 15 12 9 6"></polyline></svg>
            </div>
        )
    }
