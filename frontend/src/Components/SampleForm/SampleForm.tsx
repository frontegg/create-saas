...
const SampleForm: React.FC<SampleFormProps> = ({
    ...
    inputs.map( (input: IField) => {
        formValidationState[input.name] = input.validation || ''
    })

    const [state, setState] = React.useState(() => {
        const stateObject: any = {}
        inputs.map(input => {
            stateObject[input.name] = undefined
        })
        return stateObject
    })
    ...
    const [formValidation, setValidation] = React.useState(() => {
        ...
        inputs.map(input => {
            stateFormValidation[input.name] = false
        })
    ...
    const [formAlertValidation, setAlertValidation] = React.useState(() => {
        ...
        inputs.map(input => {
            stateFormAlertValidation[input.name] = false
        })
    ...
    React.useEffect(() => {
        if(isEnableChangeInputEffect) validation()
    }, [state]);
                        ...
                        ?   
                            <div className="inputGroup">
                                <div className="input-field mb-2">
                                    <div className="bg-transparent border-red-500 text-red-500 shadow border-l-4 border-current px-4 py-3 relative" role="alert">
                                        <span className="block sm:inline">{formAlertValidation[input.name][0]}</span>
                                        <span className="absolute top-0 bottom-0 right-0 px-4 py-3" onClick={e => closeAlert(e, input.name)}>
                                    ...
                                    {input.cases?.map(radio => 
                                        <div className="custom-control custom-control-inline custom-radio">
                                            <Input
                                                ...
                                                onChange={e => inputChangeHandler(e)}
                                            />
                                            <Label className={("form-check-label custom-control-label") + (formValidation[input.name] ? " text-red-500" : "")} htmlFor={input.name+radio} >{radio}</Label>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="form-group">
                                <div className="form-check custom-control custom-checkbox">
                                    <input 
                                        ...
                                        onChange={e => inputChangeHandler(e)}
                                    />
                                    <label className={("custom-control-label") + (formValidation[input.name] ? " text-red-500" : "") } htmlFor={input.name}>
                                        {input.label}
                                    </label>
                                </div>
                            </div>
